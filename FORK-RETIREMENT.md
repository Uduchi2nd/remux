...
- Row cap `dubmux_max_rows`=3 filled in HQ-quality order; prefetch on playback start walks the next 10 + previous 2 episodes once per series per hour (`dubmux_prefetch_episodes`/`_previous`) and pre-muxes the next one (`dubmux_premux_next`).
...
### Dub rows played as a 0-length live stream (rows 2–3) — 2026-09-26

After the fixes above the first row played with matching audio, but the other two showed length 0, played a few seconds and could not be scrubbed. The muxer only started those muxes on the player's first GET and served ffmpeg's in-progress `EVENT` playlist; VidHub treats that as live (no duration, no seeking) and stopped after the segments it first saw. The muxes themselves take 7–40 s (stream copy; 1.5–2.3 GB releases). `bda7ea03`: the muxer's master/index GETs wait up to `DUBMUX_MASTER_WAIT_S` (50 s) for the mux to finish and serve the playlist as `VOD`; `POST /mux/{dub}/{video}/start` starts a mux without waiting, and remux calls it for every dub row when a playback request creates/refreshes them (and for the next episode's first pair, replacing the earlier GET). Measured cold on Pursuit of Jade E03 through the public host: rows 2/3 returned a finished VOD playlist after 12.6 s and 35.3 s; already-muxed rows answer in under a second. A release large enough to exceed 50 s still gets the EVENT playlist (old behaviour) on that first play. Deployed: live binary `6baf9bf6775310622834b3ce39bb8e6332207820d7eb48e2a256948856180593` (backup `/opt/remux/remux-server-patched.backup-20260926T010654Z`); a PlaybackInfo on E05 produced three `POST …/start` calls at the muxer.

### Dub rows vanished after a refresh without vnphim streams — 2026-09-26

Pursuit of Jade E01 lost all three `[+VN dub]` rows overnight. The stream refresh at 07:14 UTC came back with 8 HQ streams and no vnphim streams, so `ensure_dub_rows` had nothing to pair and the previous rows (whose muxes were complete) aged out of the source list. vnphim itself was answering 0 streams for every kkphim/ophim title: its probe through the VN MediaFlow failed ("stream dead on both paths, hidden") because the seedbox-side forwarder `~/mediaflow-vn-proxy/proxy.py` (aiohttp, tmux session `mfvnproxy`, listens 127.0.0.1:18890 behind `mediaflow-vn.geniallark.box.ca`) was wedged — `/health` timed out, 44k "Cannot write to closing transport" tracebacks in its log, while the path behind it (tailscaled CONNECT proxy → VN `tailscale serve` :8443 → LXC 100 MediaFlow) answered in 1 s. Restarting it (`bash ~/mediaflow-vn-proxy/restart.sh`, added) fixed vnphim within a minute. NB `pkill -f`/`pgrep -f` over ssh match the ssh command line itself — run such kills from a script file.

`8b4a7e59`: existing dub rows are now carried over when a refresh lists no dub streams, or when the walk cannot re-confirm a pair (muxer busy/late), as long as the HQ release is still listed and the muxer has not explicitly rejected the pair; `updated_at` is refreshed and the mux URL's `?video=` is re-pointed at the release's current URL so a swept mux can be rebuilt. The row cap still applies.

Also observed: hosted AIOStreams sometimes answers remux without the vnphim streams (8 vs 11 streams for the same id within a minute, the vnphim addon timeout there is 20 s); the carry-over covers that too.

### Persistent queue does the dub/subtitle upkeep; subtitle labels; carry-over from the DB — 2026-09-26

- `a0b7081a` + `a02e0c0f`: the on-play walk (next 10, previous 2) touches every finished mux it finds (muxer `POST …/touch`, never starts one), and the persistent stream-refresh queue now does the same after each episode refresh, pre-muxes the episode's best pair while its series is being watched, and pre-aligns the external subtitles of the sources a player would pick (dub row + first HQ) via `gate::prepare_in_background`. The queue needs the running `AppState` (subtitle preparation is request-path code): exposed as `APP_STATE` (OnceLock) in lib.rs. Verified: a playback-start event produced 6 touches + 2 mux starts at the muxer within 2 minutes.
- Subtitle labels: addon tracks `"<lang> (vnphim N)"` (N per language — vnphim usually returns two Vietnamese files), the HQ release's own text tracks presented on dub rows `"… (release)"`; `[Auto-synced]` appends as before; applied to Path (VidHub's label) and DisplayTitle (Infuse's). Unit test `external_labels_number_same_language_tracks`.
- `a5e8c626`: carry-over (`8b4a7e59`) never fired on a refresh because it looked for existing dub rows in the freshly fetched addon list; stored `[+VN dub]` rows are now read from the DB (`Media::streams` hides rows older than the refresh). A refresh during the VN MediaFlow outage (15:46 UTC) had silently dropped The Early Spring E01's rows.
- Retirement evidence: stock keeps addon-muxed rows and their retention alive from its background refresh, labels addon vs embedded subtitle tracks distinguishably in VidHub/Infuse, and does not lose synthetic rows on a refresh that returns no addon streams.

### Muxer priority queue; rows carry a track list from creation — 2026-09-26

`9571299c` + `1f540383`: `/prepare` carries a priority (0 playback, 100+n on-play walk in episode order, 300 hourly queue; +50+rank for an episode's second and later variants) and the muxer's extraction/match slots are priority gates with in-place bumping, so the next episodes of the series being watched, first dub version first, are prepared before other series' background upkeep (which had left Queen of News E3 waiting behind ~30 jobs after the AIOStreams language change). Dub rows are now built from the HQ's filename-guess probe too, so a row created before its release was probed lists the Vietnamese track immediately (E2's first row briefly showed none). Retirement evidence: stock orders addon-side preparation by viewing likelihood and describes a synthetic row's tracks from creation.

...
External subtitle fetching checks HTTP success and rejects empty/error payloads without actual timed dialogue cues. This prevents an upstream JSON error such as #75 Bad Request from becoming blank WEBVTT. Existing SRT/VTT and ASS dialogue payloads are recognized. PlaybackInfo validates candidates before advertising them; the delivery route repeats the same filtering so stream indexes stay aligned. Empty, malformed, HTML, and error bodies with no cues are hidden; 400/404/410 responses and invalid bodies are negatively cached for 15 minutes. Timeouts, 429s, and 5xx responses are not classified as bad subtitles.
...
### Seven-day external subtitle retention and empty-track filtering — 2026-09-23

Production binary SHA256 `9db6cdaab2d67c796ad720acee814735a56d82397d44f6055d11e80a1b0dbffa`. PlaybackInfo validates candidate subtitle payloads before advertising them; the delivery route repeats that filter so stream indexes remain stable. Confirmed empty/error payloads are hidden and skipped by the readiness gate. Original validated subtitle text and successful per-source results are persisted for seven days under stable item/source/track identities.

Live E18 verification: Torrentio and Usenet Vietnamese requests returned HTTP 200, aligned, 1,012 cues, and appeared as `[Auto-synced]` in PlaybackInfo. Both vnphim sources returned HTTP 200 with 1,012 cues and `no-reference`; they correctly remained unmarked. Fresh PlaybackInfo after restarting Remux restored the aligned labels. No empty E18 source was present during this check, and no physical client playback was performed.

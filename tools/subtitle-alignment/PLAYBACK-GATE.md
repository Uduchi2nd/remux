...
The stale Sootio link (source 92b139c6) redirects to `sooti.click`, which returned 404 to a HEAD and to a GET through Remux. Do not mistake that link for the selected TorBox source. The fresh PlaybackInfo/probe fallback selected Torrentio TorBox source 7a16655f. The public client path was verified: remux.uduchi.com 307 → torrentio.strem.fun 302 → api.torbox.app 307 → store-042.wnam.tb-cdn.io 206 for the first byte. TorBox served 1 KiB ranges with 206 at 0, 10, 30, 50, 70, 90, and 99 percent of the 1.279 GB file. The Vietnamese subtitle returned 200, aligned, 1,012 cues. Remux logged PlaybackInfo 200, video gate 307, subtitle 200. These show that the selected TorBox stream is reachable by range from the server network and public gateway; no successful full playback on the physical player was observed. The tablet UI bridge still reports input unavailable, so next useful reproduction is a player attempt selecting the Torrentio/TorBox entry and capture which media source/error it chooses. No content/code changes made for this comparison.

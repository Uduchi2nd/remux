...
### 30-day / 200 MiB subtitle cache policy — 2026-09-23

Production binary SHA256 `edb69cc8e3ac666246cd4aeb8ff89d5878a727fb9db2c4c0a11c3ca107862ec3` (build commit `c9894161`; canonical source commit `464f834c`). The raw-good and aligned-result disk caches now share one 200 MiB budget and 30-day expiry; oldest entries are pruned first, with a 512-file cap per cache. The separate weighted in-memory store remains capped at 128 MiB.

Live after restart: E18 PlaybackInfo returned HTTP 200 with Torrentio and Usenet tracks still labeled `[Auto-synced]`; all four external Vietnamese routes returned HTTP 200 with 1,012 timed cues. The two vnphim tracks remain unmarked because they have no eligible alignment reference. Disk use was 842,862 bytes across 11 persisted cache files. The 200 MiB pruning and empty/error-payload tests passed. LXC 111 rootfs is now 12 GiB with 2.3 GiB available; disk config backup is `/root/remux-patch/lxc111-pre-subtitle-cache-resize-20260923.conf`.
